...
const object_pool_identifier = Symbol("window.object_pool")
...
function JSModule(name::Symbol, url::String)
    ...
    onimport(scope, @js function (mod)
        ...
        window.object_pool[$(uuidstr(mod))] = mod
        window.object_pool[$(uuidstr(document))] = document
    end)
    return mod, document
end
